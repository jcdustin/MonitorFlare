          ...
          <div class="flex flex-col lg:flex-row lg:items-start lg:justify-between gap-5">
            <div class="min-w-0 flex-1">
              ...
              <div v-if="monitor.cert_expiry || monitor.domain_expiry" class="mt-3 flex flex-wrap items-center gap-2">
                ...
                <span v-if="monitor.type === 'http' && monitor.check_ssl === 1 && !monitor.cert_expiry"
                  class="text-[11px] font-mono"
                  :class="monitor.info_status === 'ERROR' || monitor.info_status === 'PARTIAL' ? 'text-amber-500' : 'text-slate-400 dark:text-slate-600'"
...
const typeKey = computed(() => {
    ...
    if (type === 'http') {
        if (monitor.value?.cert_expiry) return 'ssl';
        return 'http';
    }
